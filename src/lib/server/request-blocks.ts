...
/**
 * Validates and freezes the blocked request policy list so proxy-level request
 * blocking stays declarative and extensible as more reserved path prefixes are
 * added.
 */
function defineBlockedRequestPolicies(
  ...
  for (const policy of policies) {
    ...
    if (policy.pagePath.trim().length === 0 || !policy.pagePath.startsWith("/")) {
      throw new Error(
...
/**
 * Canonical list of reserved path prefixes rejected by the proxy runtime.
 */
...
/**
 * Creates the canonical proxy block response for the matched request policy.
 * HTML callers are rewritten to the generic forbidden page while non-HTML
 * callers receive a structured JSON error payload.
 */
...
/**
 * Returns the first blocked request policy that matches the requested
 * pathname, or null when the path is allowed to continue through the proxy.
 */
export function matchBlockedRequestPolicy(
  pathname: string,
): BlockedRequestPolicy | null {
  for (const policy of blockedRequestPolicies) {
    if (policy.pathPrefixes.some((pathPrefix) => pathname.startsWith(pathPrefix))) {
      return policy;
    }
  }
