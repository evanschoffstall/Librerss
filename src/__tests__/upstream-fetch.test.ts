...
import { GotScrapingError } from "@/lib/extract/fingerprint-fetch";
import { fetchHtml } from "@/lib/extract/upstream-fetch";
...
describe("fetchHtml", () => {
  ...
  describe("proxy mode with fingerprint fetch", () => {
    ...
    test("passes proxy configuration to fingerprint fetch", async () => {
      const mockFingerprintFetch = mock(async (_url, _validator, options) => {
        ...
        expect(options?.operatingSystem).toBe(PROXY_FINGERPRINT_POOL[0].os);
  ...
  describe("proxy mode retry logic", () => {
    ...
    test("rotates fingerprints across retry attempts in proxy mode", async () => {
      ...
      const mockFingerprintFetch = mock(async (_url, _validator, options) => {
        usedFingerprints.push(options?.operatingSystem ?? "unknown");
      ...
      expect(usedFingerprints).toEqual([
        PROXY_FINGERPRINT_POOL[0].os,
        PROXY_FINGERPRINT_POOL[1 % PROXY_FINGERPRINT_POOL.length].os,
        PROXY_FINGERPRINT_POOL[2 % PROXY_FINGERPRINT_POOL.length].os,
      ]);
    });
