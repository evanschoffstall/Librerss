...
describe("SOCKS_PROTOCOLS", () => {
  test("contains all supported SOCKS protocol variants", async () => {
    const { SOCKS_PROTOCOLS } = await import("@/lib/extract/proxy-config");
  ...
  test("does not contain HTTP or HTTPS protocols", async () => {
    const { SOCKS_PROTOCOLS } = await import("@/lib/extract/proxy-config");
...
describe("buildProxyConfig", () => {
  describe("HTTP proxies", () => {
    test("parses basic HTTP proxy with explicit port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("uses default port 8080 for HTTP proxy without port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses HTTP proxy with authentication", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("http://user:pass@proxy.example.com:3128");
    ...
    test("decodes URL-encoded credentials", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("handles username without password", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses HTTP proxy with IPv4 host", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses HTTP proxy with IPv6 host", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
  ...
  describe("HTTPS proxies", () => {
    test("parses basic HTTPS proxy with explicit port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("uses default port 443 for HTTPS proxy without port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses HTTPS proxy with authentication", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
  ...
  describe("SOCKS proxies", () => {
    test("parses SOCKS4 proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS4A proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS5 proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS5H proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses generic SOCKS proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS proxy with authentication", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS proxy with IPv4 address", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("parses SOCKS proxy with IPv6 address", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
  ...
  describe("allowInsecureTls parameter", () => {
    test("respects allowInsecureTls=false for HTTP proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("respects allowInsecureTls=true for HTTP proxy", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("applies allowInsecureTls=true to SOCKS proxy agent", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
      ...
      if (config !== false && config.mode === "socks") {
        ...
        // Verify that the agent's connect method was wrapped
        // (we can't directly test the override without making an actual connection,
        // but we can verify the agent exists and has a connect method)
        expect(typeof config.httpAgent.connect).toBe("function");
        expect(typeof config.httpsAgent.connect).toBe("function");
      }
    });

    test("applies allowInsecureTls=false to SOCKS proxy agent", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("defaults allowInsecureTls to false when not provided", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
  ...
  describe("error handling", () => {
    test("returns false for invalid URL", async () => {
      ...
      const config = buildProxyConfig("not a valid url");
    ...
    test("handles malformed URL without protocol as HTTP", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("returns false for malformed URL with spaces", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("returns false for FTP protocol", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("ftp://proxy.example.com:21");

      expect(config).not.toBe(false);
      // FTP is technically a valid URL but will be treated as HTTP mode
      // since it's not in SOCKS_PROTOCOLS
      if (config !== false) {
        expect(config.mode).toBe("http");
      }
    });

    test("returns false for URL with only protocol", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("http://");

      expect(config).toBe(false);
    });

    test("returns false for URL with invalid port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("http://proxy.example.com:notaport");

      expect(config).toBe(false);
    });

    test("handles URL with port 0", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("returns false for URL with negative port", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("http://proxy.example.com:-1");

      expect(config).toBe(false);
    });

    test("returns false for URL with very large port number", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");

      const config = buildProxyConfig("http://proxy.example.com:65536");
  ...
  describe("edge cases", () => {
    ...
    test("handles uppercase protocol", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("handles mixed case hostname", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("strips protocol colon correctly", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("handles password with equals sign", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("handles empty password with colon", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
  ...
  describe("type discrimination", () => {
    test("HTTP mode config has no httpAgent or httpsAgent", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("SOCKS mode config has no proxy object", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
    ...
    test("returns same agent instance for http and https in SOCKS mode", async () => {
      const { buildProxyConfig } = await import("@/lib/extract/proxy-config");
