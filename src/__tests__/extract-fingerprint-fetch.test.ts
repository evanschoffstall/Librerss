...
describe("parseSocksProxy", () => {
  ...
  test("decodes URL-encoded username", () => {
    const result = parseSocksProxy("socks5://user%40name:pass@proxy.local:9050");
    expect(result.userId).toBe("user@name");
  });

  test("decodes URL-encoded password", () => {
    const result = parseSocksProxy("socks5://user:p%40ss%23word@proxy.local:9050");
...
describe("fetchHtmlWithFingerprint", () => {
  ...
  test("throws error when exceeding redirect limit", async () => {
    ...
    await expect(
      fetchHtmlWithFingerprint("https://example.com/start", isAllowedUrl, {}, { requestFn }),
  ...
  test("throws error on redirect without location header", async () => {
    ...
    await expect(
      fetchHtmlWithFingerprint("https://example.com", isAllowedUrl, {}, { requestFn }),
  ...
  test("throws GotScrapingError on non-2xx status", async () => {
    ...
    try {
      await fetchHtmlWithFingerprint("https://example.com", isAllowedUrl, {}, { requestFn });
...
describe("integration: generateBrowserHeaders edge cases", () => {
  ...
  test("maintains header order consistency across calls", () => {
    const headers1 = generateBrowserHeaders("2", { referer: "https://example.com" });
    const headers2 = generateBrowserHeaders("2", { referer: "https://example.com" });
