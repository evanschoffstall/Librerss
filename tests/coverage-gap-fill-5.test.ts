...
// ── app/api/auth/session/route.ts – authenticated + unauthenticated ──────────
...
describe("auth session route", () => {
  test("returns unauthenticated when no user", async () => {
    ...
    mock.module("@/lib/server", () => ({
      logAndRespondError: (_msg: string, err: unknown) =>
        Response.json({ error: String(err) }, { status: 500 }),
    }));
    const { GET } = await import("@/app/api/auth/session/route");
    const req = createMockRequest("https://host/api/auth/session");
...
// ── app/api/auth/login/route.ts – successful login ───────────────────────────

describe("auth login route – successful login", () => {
  test("POST returns user on valid credentials", async () => {
    mock.module("@/lib/auth/credentials", () => ({
      normalizeEmailInput: (e: unknown) => String(e).toLowerCase().trim(),
    }));
    mock.module("@/lib/auth/session", () => ({
    ...
    const req = createMockRequest("https://host/api/auth/login", {
      method: "POST",
      body: { email: "login@test.com", password: "SecurePass1!" },
      headers: { "sec-fetch-site": "same-origin" },
    });
