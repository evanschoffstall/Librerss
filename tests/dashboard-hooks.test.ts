...
describe("useFeedLoader", () => {
  ...
  test("bypasses the cached feed source tree when a caller requests a fresh reload", async () => {
    const categoriesRef = { current: [] as CategoryTreeNode[] };
    const cachedCategories: CategoryTreeNode[] = [
      {
        children: [],
    ...
    try {
      ...
      expect(resolvedCategories).toHaveLength(1);
      expect(resolvedCategories[0]?.label).toBe("Fresh");
      expect(resolvedCategories[0]?.children).toHaveLength(1);
      expect(resolvedCategories[0]?.children?.[0]?.label).toBe("Fresh Feed");
      expect(setCategories).toHaveBeenCalledWith(resolvedCategories);
      expect(FeedService.getFeedSources).toHaveBeenCalledTimes(1);
      const refreshedQueryData = queryClient.getQueryData<CategoryTreeNode[]>(
        getFeedSourceTreeQueryKey(false),
      );

      expect(refreshedQueryData).toBeDefined();
      expect(refreshedQueryData ?? []).toEqual(resolvedCategories);
    } finally {
      queryClient.clear();
    }
  });
